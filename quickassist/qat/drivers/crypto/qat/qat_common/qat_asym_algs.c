...
static int qat_dh_init_tfm(struct crypto_kpp *tfm)
{
	...
	kpp_set_reqsize(tfm, sizeof(struct qat_asym_request));
...
static int
qat_rsa_set_n(struct qat_rsa_ctx *ctx, const char *value, size_t vlen)
{
	struct qat_crypto_instance *inst = ctx->inst;
	struct device *dev = &GET_DEV(inst->accel_dev);
...
static int
qat_rsa_set_e(struct qat_rsa_ctx *ctx, const char *value, size_t vlen)
{
	struct qat_crypto_instance *inst = ctx->inst;
	struct device *dev = &GET_DEV(inst->accel_dev);
	const char *ptr = value;
...
static int
qat_rsa_set_d(struct qat_rsa_ctx *ctx, const char *value, size_t vlen)
{
	struct qat_crypto_instance *inst = ctx->inst;
	struct device *dev = &GET_DEV(inst->accel_dev);
	const char *ptr = value;
...
static int qat_rsa_init_tfm(struct crypto_akcipher *tfm)
{
	...
	crypto_akcipher_alg(tfm)->reqsize = sizeof(struct qat_asym_request);
#else
	akcipher_set_reqsize(tfm, sizeof(struct qat_asym_request));
#endif
 ...
 */
	...
	.reqsize = sizeof(struct qat_asym_request),
#endif
	.base = {
		.cra_name = "rsa",
